...
3. ~~音長の`-`記法~~ 完了。`r-2.`は「音長の先頭の符号」で、
   `-`はタイムポインタを後退させる（`+`は通常通り前進、`r+n`=`rn`）。
   Pascal版はタイムポインタが負になることを許し、SMF書き出し時に
   直前のイベント基準で1件ずつクランプすることで、後続イベントとの
   相対関係を保つ。現行Rustの`checked_time`も同じ挙動に変更した
...
6. ~~配列の動的な拡張（範囲外の非負添字への代入で中間要素を0として補完）~~ 完了。
7. ~~`#`マクロ本体内で失敗するケース~~ 完了。`Rythm`内で展開した文字列MMLマクロも
   リズムモードを引き継ぐようにし、`test_01.mml`が通過。
8. ~~`hy_joe.mml`の大量の回復可能エラー~~ 完了。`p%.onTime`、`y11=127`、式休符、
   stepモードの明示音長と`+`/`-`算術に対応し、トップレベル13曲すべてがエラーなしで通過。
