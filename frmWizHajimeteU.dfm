object frmWizHajimete: TfrmWizHajimete
  Left = 221
  Top = 157
  Width = 431
  Height = 210
  Caption = #31777#21336#12469#12463#12521#12539#12454#12452#12470#12540#12489
  Color = clBtnFace
  Font.Charset = SHIFTJIS_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #65325#65331' '#65328#12468#12471#12483#12463
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 12
  object imgBack: TImage
    Left = 0
    Top = 0
    Width = 423
    Height = 176
    Align = alClient
    Picture.Data = {
      }
    Stretch = True
  end
  object Shape1: TShape
    Left = 3
    Top = 3
    Width = 414
    Height = 54
    Pen.Style = psClear
  end
  object lblTitle: TLabel
    Left = 8
    Top = 10
    Width = 24
    Height = 12
    Caption = #26354#21517
    Transparent = True
  end
  object Label1: TLabel
    Left = 224
    Top = 10
    Width = 24
    Height = 12
    Caption = #20316#32773
    Transparent = True
  end
  object Label2: TLabel
    Left = 8
    Top = 34
    Width = 37
    Height = 12
    Caption = #12467#12513#12531#12488
    Transparent = True
  end
  object Label3: TLabel
    Left = 8
    Top = 74
    Width = 32
    Height = 12
    Caption = #12486#12531#12509
    Transparent = True
  end
  object Label4: TLabel
    Left = 112
    Top = 74
    Width = 12
    Height = 12
    Caption = #36933
    Transparent = True
  end
  object Label5: TLabel
    Left = 244
    Top = 74
    Width = 12
    Height = 12
    Caption = #36895
    Transparent = True
  end
  object Label6: TLabel
    Left = 264
    Top = 72
    Width = 141
    Height = 12
    Caption = '('#28436#22863#12377#12427#21069#12395#22793#26356#12375#12414#12377#12290')'
    Font.Charset = SHIFTJIS_CHARSET
    Font.Color = clWhite
    Font.Height = -12
    Font.Name = #65325#65331' '#65328#12468#12471#12483#12463
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label7: TLabel
    Left = 8
    Top = 112
    Width = 48
    Height = 12
    Caption = #25293#23376#35352#21495
    Transparent = True
  end
  object Label8: TLabel
    Left = 100
    Top = 112
    Width = 24
    Height = 12
    Caption = #20998#12398
    Transparent = True
  end
  object Label9: TLabel
    Left = 168
    Top = 112
    Width = 24
    Height = 12
    Caption = #25293#23376
    Transparent = True
  end
  object Label10: TLabel
    Left = 8
    Top = 144
    Width = 12
    Height = 12
    Caption = #35519
    Transparent = True
  end
  object edtTitle: TEdit
    Left = 56
    Top = 8
    Width = 137
    Height = 20
    TabOrder = 0
  end
  object edtComposer: TEdit
    Left = 272
    Top = 8
    Width = 137
    Height = 20
    TabOrder = 1
  end
  object edtComment: TEdit
    Left = 56
    Top = 32
    Width = 353
    Height = 20
    TabOrder = 2
  end
  object scrTempo: TScrollBar
    Left = 128
    Top = 72
    Width = 113
    Height = 20
    Max = 250
    Min = 50
    PageSize = 0
    Position = 120
    TabOrder = 3
    OnChange = scrTempoChange
  end
  object edtTempo: TEdit
    Left = 64
    Top = 72
    Width = 33
    Height = 20
    TabOrder = 4
    Text = '120'
    OnChange = edtTempoChange
  end
  object btnOK: TButton
    Left = 336
    Top = 144
    Width = 75
    Height = 25
    Caption = #27770#23450'(&O)'
    TabOrder = 5
    OnClick = btnOKClick
  end
  object edtOya: TEdit
    Left = 64
    Top = 112
    Width = 33
    Height = 20
    TabOrder = 6
    Text = '4'
    OnChange = edtTempoChange
  end
  object edtKo: TEdit
    Left = 128
    Top = 112
    Width = 33
    Height = 20
    TabOrder = 7
    Text = '4'
    OnChange = edtTempoChange
  end
  object cmbKeyFlag: TComboBox
    Left = 64
    Top = 144
    Width = 225
    Height = 20
    Font.Charset = SHIFTJIS_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #65325#65331' '#12468#12471#12483#12463
    Font.Style = []
    ItemHeight = 12
    ItemIndex = 0
    ParentFont = False
    TabOrder = 8
    Text = #35519'#()             // C'
    Items.Strings = (
      #35519'#()             // C'
      #35519'#(fcgdaeb)      // C#'
      #35519#9837'(beadg)       // D'#9837
      #35519'#(fc)           // D'
      #35519'=(1,1,2,1,1,2,1)// D#'
      #35519#9837'(bea)         // E'#9837
      #35519'#(fcgd)         // E'
      #35519#9837'(b)           // F'
      #35519'#(fcgdae)       // F#'
      #35519#9837'(beadgc)      // G'#9837
      #35519'#(f)            // G'
      #35519'=(1,1,1,1,1,2,1)// G#'
      #35519#9837'(bead)        // A'#9837
      #35519'#(fcg)          // A'
      #35519'=(1,1,2,1,1,2,2)// A#'
      #35519#9837'(be)          // B'#9837
      #35519'#(fcgda)        // B'
      #35519#9837'(bea)         // Cm'
      #35519'#(fcgd)         // C#m'
      #35519'=(-1,-2,-1,-1,-1,-1,-1)// D'#9837'm'
      #35519#9837'(b)           // Dm'
      #35519'#(fcgdae)       // D#m'
      #35519#9837'(beadgc)      // E'#9837'm'
      #35519'#(f)            // Em'
      #35519#9837'(bead)        // Fm'
      #35519'#(fcg)          // F#m'
      #35519'=(-1,-2,-1,-1,-2,-1,-1)// G'#9837'm'
      #35519#9837'(be)          // Gm'
      #35519'#(fcgda)        // G#m'
      #35519#9837'(beadgcf)     // A'#9837'm'
      #35519'#()             // Am'
      #35519'#(fcgdaeb)      // A#m'
      #35519#9837'(beadg)       // B'#9837'm'
      #35519'#(fc)           // Bm')
  end
end
