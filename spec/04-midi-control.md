...
`{...}`で囲んだ値はそのまま出力し、直後に`128 - (合計 mod 128)`で計算したチェックサムを
1バイト追加します。上の例のチェックサムは`$0A`です。`SysEx$`内の裸の数値は16進数、
括弧内の変数・式は通常の数値として評価します。
...
`!1^1`のように`^`で後続の音長を結合でき、この例は全音符2個分になります。
...
`CCMute(on)` は現在のトラックでCCとピッチベンドの書き込みを抑制し、`CCMute(off)` で再開します。
値の状態はミュート中も更新されるため、後続の `MML(y11)` などから参照できます。

`CCNoMute(no,on)` は指定した0〜127番のCCだけを抑制し、`CCNoMute(no,off)` で再開します。
ピッチベンドを含む全体の抑制には `CCMute` を使います。

`DeleteCC(no)` は、現在位置以降に予約済みの指定CCを現在のトラックから削除します。
`DeleteCC(256)` と `DeleteCC(257)` は、どちらも同じチャンネルのピッチベンドを対象にします。
過去のイベントは削除しません。
