...
interface
uses
  SysUtils, Classes, Windows, StrUnit, filectrl, grids {$IFDEF VER140},Variants{$ENDIF}{$IFDEF VER150},Variants{$ENDIF};
...
var
    ...
    begin
        ...
        if JPosEx('ID', s, 1) > 0 then Result := True else // Excel対策 Excelで、IDと書くとエラーが出る
        if JPosEx(#13, s, 1) > 0 then Result := True else
        if JPosEx(#10, s, 1) > 0 then Result := True;
    end;
...
begin
    ...
    begin
        if (VarToDateTime(Cells[ColNo,i1]) <= VarToDateTime(Cells[ColNo,i2])) then
        begin
...
// 計算用関数テーブル
    function f_eq_s(const a,b: string): Boolean; begin Result := WildMatchFilename(a,b);  end;
    function f_ne_s(const a,b: string): Boolean; begin Result := not(WildMatchFilename(a,b)); end;
...
begin
    ...
    begin //文字比較
        ...
        begin
            c := rowList.Items[i];
            if (c<>nil) then begin
                Result[i] := funcStrArray[flagNo](c.Items[fno], value);
            end else
                Result[i] := False;
        end;
    end else
...
var
    i,j: Integer;
    best_j: Integer;
    best_value: TDateTime;
begin
    for i:=min to max-1 do
    begin
        // 残りの要素から一番小さいものを見つける
        best_value := VarToDateTime(Cells[ColNo, i]);
        best_j := i;
        for j := i+1 to max do
            if (VarToDateTime(Cells[colNo,j]) < best_value) then
            begin
                best_value := VarToDateTime(Cells[ColNo, j]);
                best_j := j;
            end;
        //要素を交換する
        RowList.Exchange(i, best_j);
...
begin
    ...
    for x:=0 to ColCount-1 do //　最後データを初期化
    begin
        grd.Cells[x,RowCount] := '';
    end;
...
begin
    ...
    while i < RowList.Count do
    begin
        no := Find(Cells[ColNo, i], ColNo, i + 1);
        if no > 0 then
            DeleteRow(i)
        else
            Inc(i);
